...
class AiPublisherController extends ApiController
{
    ...
    {
        return $this->success($this->discovery->generate());
    }

    public function docs(): JsonResponse
    {
        return $this->success($this->discovery->documentation());
    }

    public function schemaPost(): JsonResponse
    {
        return $this->success($this->schema->forPost());
    }

    public function schemaCategory(): JsonResponse
    {
        return $this->success($this->schema->forCategory());
    }

    public function editorialGuide(): JsonResponse
    {
        return $this->success($this->editorial->get());
    }

    public function publish(StoreAiPostRequest $request): JsonResponse
    {
        $post = $this->publisher->publish($request->validated());

        return $this->created(PostResource::make($post), 'Post criado via IA com sucesso. Status: draft.');
    }
}
